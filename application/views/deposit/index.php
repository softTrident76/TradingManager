	...
	<div class="row page-titles mx-0">
		<div class="col-sm-6 p-md-0">
			<div class="breadcrumb-range-picker">
				...
				<span class="ml-1"><?php echo get_langdata($this->session->userdata('lang'), 'dpsthistory'); ?></span>
			</div>
		</div>
	</div>
							...
							<table class="table market-cap table-responsive-sm" id="tbltransaction">
								<thead>
									<tr>
										...
										<th><?php echo get_langdata($this->session->userdata('lang'), 'amount'); ?></th>	
...
<script>
	...
	(function($){		
		...
		// set pagination //
		displayTable(content, summupProfit);
	...
	function displayTable(records, sumprft)
	{
		// set table //
		...
		{	
			var sNode = '<tr>\
							...
							<td class="text-info">' + decimalFormat(records[idx].amount) + '</td>\
						</tr>';
			$("#tbltransaction > tbody:last-child").append(sNode);
		}

		var sNode = '<tr>\
							<td colspan="2" class="text-warning" style="text-align:right; font-weight:bold "><?php echo get_langdata($this->session->userdata('lang'), 'totalamount'); ?></td> \
							<td >'+ decimalFormat(sumprft) + '</td> \
						</tr>';
		$("#tbltransaction > tbody:last-child").append(sNode);
	...
	{
		$.ajax({
			url: '/index.php/deposit/ajaxChangePage',
			type: 'post',
			dataType: 'json',
